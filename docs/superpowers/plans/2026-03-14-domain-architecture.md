...
**Architecture:** Three namespaces (`core.*`, `domain.*.*`, `ui.*.*`) with alphabetical sort = dependency direction. FSD segments (`model/`, `api/`, `lib/`, `ui/`) internally. Two public surfaces: `domain.service.*` and `ui.page.*`. Reactivity via Effect PubSub → Stream → SolidJS signal bridge.
...
- Create: `packages/libs/ui.page.main/package.json`
- Create: `packages/libs/ui.page.main/tsconfig.json`
- Create: `packages/libs/ui.page.main/src/index.ts`
...
- [ ] **Step 7: Create ui.page.main package shell**

Same pattern with:
- `name`: `@ctrl/ui.page.main`
...
- No cross-page imports within `ui.page.*`
...
### Task 11: Implement ui.page.main

**Files:**
- Create: `packages/libs/ui.page.main/src/ui/MainPage.tsx`
- Modify: `packages/libs/ui.page.main/src/index.ts`
...
Run: `cd packages/libs/ui.page.main && bun run check`
Expected: PASS

- [ ] **Step 4: Commit**

```bash
git add packages/libs/ui.page.main/
git commit -m "feat(ui.page.main): implement main page composing sidebar feature"
```
...
- [ ] **Step 2: Update main-ui/App.tsx to use RuntimeProvider + ui.page.main**

```tsx
import { RuntimeProvider } from "@ctrl/core.ui"
import { MainPage } from "@ctrl/ui.page.main"
