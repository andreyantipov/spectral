...
- `apps/*` can only import `ui.page.*` (not `ui.feature.*`)
...
`import $_ from $path` where {
  or {
    ...
    and {
      ...
      $path <: or { includes "domain.feature.", includes "domain.service.", includes "domain.adapter.", includes "ui.feature.", includes "ui.page." }
    }
  }
}
```
