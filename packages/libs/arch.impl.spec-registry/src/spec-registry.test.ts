...
const runTest = <A>(effect: Effect.Effect<A, unknown, SpecRegistry | EventBus | FeatureRegistry>) =>
	...
	it("register spec and spawn via trigger action", async () => {
		await runTest(
			Effect.gen(function* () {
				...
				yield* features.register("do.work", () => Ref.set(called, true).pipe(Effect.as(undefined)));
