...
1. **Before/After dependency graph** — generated by `bun run docs:deps` (dep-cruiser SVG from real imports, not hand-drawn). Attach SVG from main (before) and branch (after) to PR.
